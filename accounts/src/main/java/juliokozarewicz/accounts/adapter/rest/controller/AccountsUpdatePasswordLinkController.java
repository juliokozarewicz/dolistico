...
public class AccountsUpdatePasswordLinkController {
    ...
    public ResponseEntity<StandardResponseDTO> handle (
        ...
        .status(GlobalSuccessEnum.ACCOUNTS_PASSWORD_RESET_LINK_SENT_SUCCESSFULLY.getStatusCode())
        .contentType(MediaType.APPLICATION_JSON)
        .body(
            new StandardResponseDTO.Builder()
            .timestamp(Instant.now().truncatedTo(ChronoUnit.SECONDS))
            .statusCode(GlobalSuccessEnum.ACCOUNTS_PASSWORD_RESET_LINK_SENT_SUCCESSFULLY.getStatusCode())
            .messageCode(GlobalSuccessEnum.ACCOUNTS_PASSWORD_RESET_LINK_SENT_SUCCESSFULLY.getMessageCode())
            .build()
        );
