...
public class CategoriesUpdateController {
    ...
    public ResponseEntity create (
        ...
        .status(GlobalSuccessEnum.CATEGORY_UPDATED_SUCCESSFULLY.getStatusCode())
        .contentType(MediaType.APPLICATION_JSON)
        .body(
            new StandardResponseDTO.Builder()
            .timestamp(Instant.now().truncatedTo(ChronoUnit.SECONDS))
            .statusCode(GlobalSuccessEnum.CATEGORY_UPDATED_SUCCESSFULLY.getStatusCode())
            .messageCode(GlobalSuccessEnum.CATEGORY_UPDATED_SUCCESSFULLY.getMessageCode())
            .build()
        );
